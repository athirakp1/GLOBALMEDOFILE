...
def user_registerview(request):
    ...
    if(request.POST):
        ...
        # sendsms(phone,msg)
        # sendmail(site,msg)
        return HttpResponseRedirect("/user_loginview", {"g_id":g_id})
...
def hos_leaveview(request):
    ...
    data = c.fetchall()
    a = "nurse"
    n = "select * from st_leave where hos_id='"+hos_id+"' and designation='"+a+"'"
    c.execute(n)
    data1 = c.fetchall()
    b = "staff"
...
def dr_leaveview(request):
    ...
    if(request.POST):
        ...
        data = c.fetchone()
        if(int(data[0])<=12):
            s = "insert into dr_leave(name,dr_id,`leave`,`days`,date_from,date_to,subject,reason,hos_id) values('"+str(name)+"','"+str(dr_id)+"','"+str(leave)+"','"+str(days)+"','"+str(date_from)+"','"+str(date_to)+"','"+str(subject)+"','"+str(reason)+"','"+str(hos_id)+"')"
            c.execute(s)
            conn.commit()
        else:
...
def st_leaveview(request):
    ...
    if(request.POST):
        ...
        data = c.fetchone()
        if(int(data[0])<=12):
            d = "insert into st_leave(`name`,`st_id`,`leave`,`days`,`date_from`,`date_to`,`subject`,`reason`,`hos_id`,`designation`) values('"+str(uname)+"','"+str(ug_id)+"','"+str(leave)+"','"+str(days)+"','"+str(date_from)+"','"+str(date_to)+"','"+str(subject)+"','"+str(reason)+"','"+str(hos_id)+"','"+str(designation)+"')"
            c.execute(d)
            conn.commit()
        else:
            msg="SORRY YOU HAVE ALREADY TAKEN YOUR FULL ANNUAL LEAVE"
